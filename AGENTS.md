...
1. **Run `ruff check src tests scripts` AND `pytest -q` before declaring a task done.** Both must be green. Test count is 457 and growing — but signal matters more than count, so don't add trivial tests just to inflate it. Regressions are not acceptable.
...
pytest -q                              # should print "457 passed"
python -m rin --smoke
```
