...
| `tests/` | 511 tests, all green. Run with `pytest -q`. Layout mirrors `src/rin/`. |
...
| **`TrayApp._prewarm_menu()` paid at boot** | Cold `sizeHint()` on an emoji-loaded `QMenu` is ~511 ms; pre-warming 250 ms after `tray.show()` drops first-click to ~4 ms. Reviewed in v0.7.1 (R21). | `src/rin/ui/tray.py` |
...
```powershell
pytest -q                              # all 511 tests, ~60-90 s on a warm cache
